...
function Paginate({ recordsPerPage, totalRecords, handleClick, prevPage, nextPage, currentPage }) {
    let pages = [];
    ...
    return (
        ...
        <ul className="flex items-center -space-x-px h-8 text-sm cursor-pointer">
            {
                <li>
            ...
            {
                pages.map(note => (
                    <li key={note} onClick={() => handleClick(note)}

                        className={note !== currentPage
                            ?
                            "flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-whit"
                            :
